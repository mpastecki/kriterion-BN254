...
/-- This simulator samples only the curve source and the scalar ciphertext. -/
...
def Coin.state (coin : Coin) : State := {
  ...
  target := coin.target
  ciphertext := coin.ciphertext }

def State.table (state : State) : Public :=
  ⟨state.curve.table, state.ciphertext⟩

def State.labels (state : State) (input : AffineInput) : Garbling.Labels :=
  ⟨BitInput.ofAffine input, state.inputKey.encodeAffine input⟩

def State.selectedCurve (state : State) (input : AffineInput) : CurveGateRequest :=
  state.curve.retarget input state.target

/-- The inverse is used only after a valid input and its output are supplied.
The invalid branch does not program the hidden scalar-pad hash point. -/
def State.hashState [FieldCertificate] [GroupCertificate]
    (state : State) (input : AffineInput) (output : Option Point) : SimulatorState :=
  if OnCurve input then
    tryProgramHash state.oracle state.target
      (programValue (ScalarRecovery.recover input output) state.ciphertext)
  else state.oracle
...
/-- Unlike the old simulator, fresh hash points may change. Only recorded hash
inputs are therefore marked as already seen. EncPRF permutations never change. -/
...
private theorem hashState_fixed [FieldCertificate] [GroupCertificate]
    (state : State) (input : AffineInput) (output : Option Point) :
    (state.hashState input output).fixedTranscript = state.oracle.fixedTranscript := by
  unfold State.hashState
  split
  · unfold tryProgramHash
    split <;> rfl
  · rfl

private theorem hashState_hash [FieldCertificate] [GroupCertificate]
    (state : State) (input : AffineInput) (output : Option Point) :
    state.oracle.hashTranscript ⊆ (state.hashState input output).hashTranscript := by
  unfold State.hashState
  split
  · unfold tryProgramHash
    split <;> simp [programHash, markBad]
  · exact List.Subset.refl _

private theorem hashState_enc [FieldCertificate] [GroupCertificate]
    (state : State) (input : AffineInput) (output : Option Point) :
    (state.hashState input output).encOracle = state.oracle.encOracle := by
  unfold State.hashState
  split
  · unfold tryProgramHash
    split <;> rfl
...
private theorem hashState_invariant [FieldCertificate] [GroupCertificate]
    (state : State) (input : AffineInput) (output : Option Point)
    (valid : SimulatorInvariant state.oracle) : SimulatorInvariant (state.hashState input output) := by
  unfold State.hashState
  split
  · exact tryProgramHash_preservesInvariant _ _ _ valid
  · exact valid
