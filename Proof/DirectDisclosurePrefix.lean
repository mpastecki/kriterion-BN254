...
abbrev Frame := CurvePublicSample × SimulatorOracleCoin
...
def coinEquiv : Coin ≃ Secret × Frame where
  toFun coin := ((coin.inputKey, coin.target), (coin.curve, coin.oracles))
  invFun value := {
    curve := value.2.1
    oracles := value.2.2
    inputKey := value.1.1
  ...
  right_inv value := by rcases value with ⟨⟨key, target⟩, curve, oracles⟩; rfl
...
def initialOracle (frame : Frame) : SimulatorState := {
  fixedOracle := frame.2.fixedOracle
  encOracle := frame.2.encOracle
  hashOracle := frame.2.hashOracle
  fixedTranscript := []
  encTranscript := []
  hashTranscript := []
  commitments := []
...
def frameTable (frame : Frame) : Public := frame.1.request.table
