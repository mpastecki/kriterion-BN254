...
abbrev SharedQuery := (publicOracleSpec Shared.FixedKeyIndex EncPRF.PermutationIndex).Query
abbrev SharedAnswer := (publicOracleSpec Shared.FixedKeyIndex EncPRF.PermutationIndex).Answer
abbrev sharedSpec := publicOracleSpec Shared.FixedKeyIndex EncPRF.PermutationIndex
abbrev SharedState := SimulatorState Shared.FixedKeyIndex
abbrev SharedCommand := Shared.FixedKeyIndex × Block × Block

/-- The shared source separates internal reads from checked fixed programming. -/
inductive Request where
  | read (query : SharedQuery)
  | program (command : SharedCommand)

abbrev spec : OracleSpec where
  Query := Request
  Answer
    | .read query => SharedAnswer query
    | .program _ => Unit

/-- The shared eager handler uses the actual transcript guard. -/
def handler : OracleHandler spec SharedState
  ...
  | .program command, state => ((), tryProgramFixed state command.1 command.2.1 command.2.2)

/-- The sparse family uses the exact fixed-key and EncPRF indices. -/
abbrev OracleIndex := Shared.FixedKeyIndex ⊕ EncPRF.PermutationIndex
...
structure Metadata where
  fixedTranscript : List (PermutationRecord Shared.FixedKeyIndex Block)
...
def decode (metadata : Metadata) (complete : OracleCompletion) : SharedState where
...
def Metadata.program (metadata : Metadata) (command : SharedCommand) : Metadata :=
...
noncomputable def completion (state : SparseState) : PMF SharedState :=
  (lowerCompletion state.oracles).map (decode state.metadata)

/-- An external read records the original adversary query and answer. -/
def Metadata.record : (query : SharedQuery) → SharedAnswer query → Metadata → Metadata
...
def externalDraw (query : SharedQuery) (state : SparseState) :
    Draw (SharedAnswer query × SparseState) :=
...
abbrev combinedSpec := sumSpec spec sharedSpec

/-- The eager combined handler uses the original internal and adversary handlers. -/
def combinedEager : OracleHandler combinedSpec SharedState
...
theorem decode_read (metadata : Metadata) (complete : OracleCompletion) (query : SharedQuery) :
...
theorem decode_program (metadata : Metadata) (complete : OracleCompletion) (command : SharedCommand) :
...
def recordEager : (query : SharedQuery) → SharedAnswer query → SharedState → SharedState
...
theorem decode_record (metadata : Metadata) (complete : OracleCompletion)
    (query : SharedQuery) (answer : SharedAnswer query) :
...
theorem recorded_read (query : SharedQuery) (state : SharedState) :
...
theorem external_step (query : SharedQuery) (state : SparseState) :
    (completion state).map (idealOracleHandler query) =
      (externalDraw query state).distribution.bind (fun answer =>
        (completion answer.2).map (fun eagerState => (answer.1, eagerState))) := by
  have law := congrArg
    (fun distribution : PMF (SharedAnswer query × SharedState) =>
