...
def publicHashReserve (count : Nat) (memory : Memory) : Nat := (hashScan count memory).2 + 2616

/-- The complete hash branch implements the sparse source and emits its packed answer. -/
theorem publicHandler_hashRun [BN254.FieldCertificate] (attempts count : Nat) (memory : Memory)
    (tableFits : count < 2 ^ 256)
    (counter : (oracleLoaded memory).registers 0 = BitVec.ofNat 256 count) :
    run (publicHandler attempts) (publicHashReserve count memory) ⟨542, memory⟩ =
      (hashHandlerSamples count memory).map fun result =>
        some (⟨publicTailLabel result.1, (publicHashTail result.1).1⟩,
          (publicHashTail result.1).2 + (result.2 - 1)) := by
  have executed := hashHandlerBlock_continue (publicHandler attempts) publicHashLabels
    (publicHandler_hash attempts) count 772 memory tableFits counter
  dsimp only at executed
  change run (publicHandler attempts) ((hashScan count memory).2 + 1844 + 772) ⟨542, memory⟩ = _ at executed
  rw [show publicHashReserve count memory = (hashScan count memory).2 + 1844 + 772 by
    simp [publicHashReserve, Nat.add_assoc], executed]
  ...
  change (run (publicHandler attempts) remaining ⟨615, final⟩).map _ = _
  rw [show remaining = 772 + (remaining - 772) by omega, publicHandler_hashTail, PMF.pure_map]
  rfl

/-- The complete hash branch has a linear concrete instruction budget. -/
theorem publicHashReserve_bound (count : Nat) (memory : Memory) :
    publicHashReserve count memory ≤ 6 * count + 2619 := by
  have bound := tableScan_cost count (tableInitial (hashReady (oracleLoaded memory)))
  dsimp [publicHashReserve, hashScan]
  omega
