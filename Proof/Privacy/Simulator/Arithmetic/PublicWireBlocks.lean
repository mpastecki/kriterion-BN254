...
/-- A present coefficient emits exactly the stored value, with no per-field tag. -/
theorem wireCoefficient_bits (ram : Word → Word) (pointer : Word) (base index : Nat) (value : BaseField)
    ...
    (wireCoefficient base index).flatMap (WireSegment.wire ram pointer) =
      (Wire.field.encode value).flatMap (fun byte => bits 8 byte.val) := by
  simp only [wireCoefficient, List.flatMap_cons, List.flatMap_nil, List.append_nil]
  exact wireStored_field ram pointer (base + index) value stored

/-- A present digit table emits exactly its stored words, with no per-field tag. -/
theorem wireOptionalDigit_bits (ram : Word → Word) (pointer : Word) (base index : Nat)
    ...
    (wireOptionalDigit base index).flatMap (WireSegment.wire ram pointer) =
      (Wire.digit.encode value).flatMap (fun byte => bits 8 byte.val) := by
  exact wireDigit_bits ram pointer (base + 254 * index) value stored

/-- The row header emits exactly its one-byte schema tag. -/
theorem wireHeader_bits (ram : Word → Word) (pointer : Word) (schema : Fin 256) :
    (WireSegment.wire ram pointer (.tag schema)) = bits 8 schema.val := by
  rfl
