...
/-- The canonical input reader preserves the complete programmed memory relation. -/
theorem publicInputMemory_programmed (memory : Memory)
    ...
    ProgrammedMemory (publicInputMemory memory request rest).ram oracle state := by
  rw [(publicInputMemory_values memory request rest).2.2.2.2]
  exact represented

/-- The actual public machine implements both forward query constructors and their operational cutoff law. -/
theorem publicHandler_forwardSource [BN254.FieldCertificate] (attempts : Nat) (memory : Memory)
    (request : PublicQuery Shared.FixedKeyIndex EncPRF.PermutationIndex) (rest : List Bool)
    ...
    (run (publicHandler attempts)
      (publicHandlerReserve attempts state.base.used state.overlay.length memory request rest) ⟨0, memory⟩).map
  ...
  have values := publicInputMemory_values memory request rest
  have stored := publicInputMemory_programmed memory request rest oracle state represented
  have counts : PublicBranchCounts (publicHandlerKind request) attempts state.base.used state.overlay.length
      (publicInputMemory memory request rest) := by
    rw [forward]
  ...
  have overlays : state.overlay.length < 2 ^ 256 := by
    ...
    omega
  rw [publicHandler_answers attempts state.base.used state.overlay.length memory request rest wire empty
    attemptFits tableFits overlays counts]
  have source := programmedForwardReplies_typed request attempts (publicInputMemory memory request rest) oracle
    state input stored (values.2.1.trans index) (values.1.trans operand) fits overlayFits
  cases request with
  | fixedForward index value => simpa only [publicHandlerReplies] using source
  | encForward index value => simpa only [publicHandlerReplies] using source
...
theorem publicHandler_inverseSource [BN254.FieldCertificate] (attempts : Nat) (memory : Memory)
    (request : PublicQuery Shared.FixedKeyIndex EncPRF.PermutationIndex) (rest : List Bool)
  ...
  rw [publicHandler_answers attempts state.base.used state.overlay.length memory request rest wire empty
    attemptFits tableFits overlays counts]
  have source := programmedInverseReplies_typed request attempts (publicInputMemory memory request rest) oracle
    state input stored (values.2.1.trans index) (values.1.trans operand) fits overlayFits
  cases request with
  | fixedInverse index value => simpa only [publicHandlerReplies] using source
  | encInverse index value => simpa only [publicHandlerReplies] using source
...
theorem publicHandler_hashSource [BN254.FieldCertificate] (attempts : Nat) (memory : Memory)
    ...
    (run (publicHandler attempts)
      (publicHandlerReserve attempts pairs.length 0 memory (.hash key) rest) ⟨0, memory⟩).map
  ...
  have values := publicInputMemory_values memory request rest
  have stored : HashMemory (publicInputMemory memory request rest).ram 15748
      (hashWordPairs (fun value => BitVec.ofNat 256 value.val) pairs) := by
    rw [values.2.2.2.2]
    exact represented
  have index : (publicInputMemory memory request rest).registers 9 = BitVec.ofNat 256 (15748 : Fin 15749).val := values.2.1
  have counts : PublicBranchCounts .hash attempts pairs.length 0 (publicInputMemory memory request rest) := by
    simpa only [hashWordPairs, List.length_map] using
      publicHandler_hashCounts attempts 0 (publicInputMemory memory request rest) 15748 _ stored index
  have tableFits : pairs.length < 2 ^ 256 := by
    have large : 2 ^ 110 < 2 ^ 256 := by decide
    omega
  rw [publicHandler_answers attempts pairs.length 0 memory request rest wire empty attemptFits tableFits (by decide) counts]
  exact hashReplies_typed pairs key (publicInputMemory memory request rest) 15748 stored index values.1 fits
